...
///   DsStatusBadge(status: 'completed', icon: Icons.check_circle)
...
class DsPaymentBadge extends StatelessWidget {
  final String status;

  const DsPaymentBadge({super.key, required this.status});

  @override
  Widget build(BuildContext context) {
    final (color, subtle, border, label, icon) = switch (status.toLowerCase()) {
      'paid' => (
          AppColors.success,
